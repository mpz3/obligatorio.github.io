...
document
  ...
  .addEventListener("click", liMostrarNuevaSolicitudUI);
document
  .querySelector("#liConsultarSolictudes")
  .addEventListener("click", liMostrarConsultarSolictudesUI);
document
  .querySelector("#liCancelarSolicitud")
  .addEventListener("click", liMostrarCancelarSolicitudUI);
document
  .querySelector("#liCrearViaje")
  .addEventListener("click", liCrearViajeUI);
document
  .querySelector("#liAsignarBuque")
  .addEventListener("click", liAsignarBuqueUI);
document.querySelector("#liRollover").addEventListener("click", liRolloverUI);
document
  .querySelector("#liManifiesto")
  .addEventListener("click", liManifiestoUI);
document.querySelector("#liHablitarI").addEventListener("click", liHablitarIUI);
document
  .querySelector("#liListaPeligrosa")
  .addEventListener("click", liListaPeligrosaUI);
document
...
document
  .querySelector("#liEstadistica")
  .addEventListener("click", estadisticaUI);
document
  .querySelector("#btnNuevoViaje")
  .addEventListener("click", btnNuevoViajeUI);
document
  .querySelector("#btnConfirmarCarga")
  .addEventListener("click", btnConfirmarCargaUI);
...
function registroUI() {
  ...
  if (!registroValido) {
    ...
    while (!userEncontrado && i < usuarios.length) {
      if (usuarios[i].user.toLowerCase() === usuario.toLowerCase()) {
...
function estadisticaUI() {
  ...
  ).innerHTML = `El porcentaje de cancelaciones contra el total de cargas es ${porcentaje.toFixed(
    2
  )}%`;
}
...
function btnNuevoViajeUI() {
  ...
  if (nombreB != "" && cantMax != "" && !isNaN(cantMax) && fecha != "") {
    ingresarBuque(nombreB, cantMax, fecha, userOnline);
    document.querySelector(
      "#divMostrarAutomatico"
    ).innerHTML = `Se ingreso correctamente con el id ${
      buques[buques.length - 1].id
    }`;
  } else {
    document.querySelector(
      "#divMostrarAutomatico"
    ).innerHTML = `Los campos son invalidos`;
  }
}
...
function btnConfirmarCargaUI() {
  ...
  if (
    ...
    document.querySelector(
      "#pIDSolicitudConfirmada"
    ).innerHTML = `Se confirmo Correctamente`;
  } else {
    document.querySelector(
      "#pIDSolicitudConfirmada"
    ).innerHTML = `Todos los campos son requeridos`;
  }
  // aca pongo el rolleback para que se carguen las opciones una vez que se aprueba la solicitud.
  mostrarRolleback();
}
//
function rolleback() {
  let valRolleback = document.querySelector("#selRollover").value;
  document.querySelector("#divMostrarTablaRollover").innerHTML =
    mostrarCargasRolleback(valRolleback);
}
document.querySelector("#btnRollover").addEventListener("click", rolleback);
