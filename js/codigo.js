...
function buscarImportador(user, pass) {
  ...
  while (!encontrado && i < importadores.length) {
    if (
      importadores[i].user.toLowerCase() === user &&
      importadores[i].contraseña === pass
    ) {
      encontrado = true;
    }
    i++;
...
function validarDatosMercaderia(
  ...
  if (
    ...
    if (pidEmpresa === "" || isNaN(pidEmpresa))
...
function busquedaSolicitudesPendientes(busqueda) {
  ...
  for (let i = 0; i < solicitudesDeCarga.length; i++) {
    if (
      solicitudesDeCarga[i].id === Number(busqueda) ||
      solicitudesDeCarga[i].descripcion.toLowerCase() === busqueda.toLowerCase()
    ) {
...
function buscarSolcitudesAprobadasBuques()
