...
function buscarUser(pUser, pPass) {
  ...
  while (!encontrado && i < usuarios.length) {
    if (
      usuarios[i].user.toLowerCase() === pUser &&
      usuarios[i].contraseña === pPass
    ) {
      encontrado = true;
...
function validarContrasena(pPassw) {
  ...
  while (
    i < pPassw.length ||
    (!mayusEncontrada && !minusEncontrada && !numEncontrado)
  ) {
...
function validarDatosMercaderia(
  pDesc,
  pTipo,
  pPuerto,
  pCantContenedores,
  ...
  if (
    ...
    pPuerto === "" ||
    pCantContenedores === "" ||
    isNaN(pCantContenedores) ||
    pIEmpresa === "" ||
    isNaN(pIEmpresa)
  ) {
    if (pDesc === "")
      document.querySelector("#txtDescrip").style.borderColor = "red";
    else document.querySelector("#txtDescrip").style.borderColor = "black";

    if (pTipo === "")
      document.querySelector("#txtTipoCarga").style.borderColor = "red";
    else document.querySelector("#txtTipoCarga").style.borderColor = "black";

    if (pPuerto === "")
      document.querySelector("#txtPuerto").style.borderColor = "red";
    else document.querySelector("#txtPuerto").style.borderColor = "black";

    if (pCantContenedores === "" || isNaN(pCantContenedores))
      document.querySelector("#txtCantContenedores").style.borderColor = "red";
    else
      document.querySelector("#txtCantContenedores").style.borderColor =
        "black";

    if (pIEmpresa === "" || isNaN(pIEmpresa))
      document.querySelector("#txtIdEmpresa").style.borderColor = "red";
...
function busquedaSolicitudesPendientes(pBusqueda) {
  ...
  for (let i = 0; i < solicitudesDeCarga.length; i++) {
    if (
      solicitudesDeCarga[i].id === Number(pBusqueda) ||
      solicitudesDeCarga[i].descripcion.toLowerCase() ===
        pBusqueda.toLowerCase()
    ) {
...
function cargarDatosPendientes() {
  ...
  for (let i = 0; i < buques.length; i++) {
    ...
    if (
      new Date(buque.fechaLlegada) > new Date() &&
      buque.cargaMaxima >= solicitudesDeCarga[i].cantidadContenedores
    ) {
...
// comprobar si funciona la validacion de repetidos. ya que si cargaba cada viaje se iban a repetir los ids, de esta forma creo, que evitamos ver más de un id de viaje
function mostrarRolleback() {
  let mostrarSelect = ``;
  let i = 0;
  let viajeYaCargado = new Array();
  while (i < viajesConfirmados.length) {
    let opcionesRoll = viajesConfirmados[i];
    for (x = 0; x < viajeYaCargado.length; x++) {
      if (viajeYaCargado[x] != opcionesRoll.idViaje) {
        viajeYaCargado.push(opcionesRoll.idViaje);
        mostrarSelect = `<option value = "${opcionesRoll.idViaje}"> ${opcionesRoll.idViaje}`;
      }
    }
    i++;
  }
  return (document.querySelector("#selRollover").innerHTML = mostrarSelect);
}

function mostrarCargasRolleback(pRolleback) {
  let tablaRolleback = `<table> <tr><th>ID carga</th></tr> <tr><th>ID Viaje</th></tr> <tr><th> Rollback</th></tr>`;
  let i = 0;
  while (i < viajesConfirmados.length) {
    let rollebackX = viajesConfirmados[i];
    if (rollebackX.idViaje === pRolleback) {
      tablaRolleback += `<tr><td>${rollebackX.idCarga} </td></tr> <tr><td>${rollebackX.idViaje}</td></tr> <tr><td> <input type="button" value="Rollback" id="btnRollover"> </td></tr>`;
    }
    i++;
  }
  tablaRolleback += `</table>`;
  return tablaRolleback;
}
